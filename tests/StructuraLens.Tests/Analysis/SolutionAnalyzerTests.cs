...
public class SolutionAnalyzerIntegrationTests
{
    ...
    [Test]
    public async Task AnalyzeProjectAsync_WithNUnitSetupInitializedField_DoesNotReportCS8618()
    {
        var tempDir = Path.Combine(Path.GetTempPath(), $"StructuraLensNUnitSuppressionTest_{Guid.NewGuid():N}");

        try
        {
            Directory.CreateDirectory(tempDir);

            var projectFilePath = Path.Combine(tempDir, "NUnitSuppression.csproj");
            var sourceFilePath = Path.Combine(tempDir, "SuppressionFixture.cs");
            ...
            var sourceContent = """
                ...
                [TestFixture]
                public sealed class SuppressionFixture
                {
                    private object _checkoutController;

                    [SetUp]
                    public void SetUp()
                    {
                        _checkoutController = new object();
                    }

                    [Test]
                    public void Works()
                    {
                        Assert.That(_checkoutController, Is.Not.Null);
                    }
                }
                """;
            ...
            var report = await analyzer.AnalyzeProjectAsync(projectFilePath);
            var project = report.Projects.Single();
            await Assert.That(project.Diagnostics).IsNotNull();

            var hasCs8618 = project.Diagnostics!.Diagnostics.Any(d => d.Id == "CS8618");

            await Assert.That(hasCs8618).IsFalse();
        }
        finally
        {
            if (Directory.Exists(tempDir))
